...
if (!function_exists('nxs_settings_open')){ function nxs_settings_open() {  
	$fileData = trim(file_get_contents(dirname(__DIR__).'/5a8291484e424a1e7c093af0-nx-snap-settings.txt'));  $options = nxs_maybe_unserialize($fileData);  return $options;
}}
if (!function_exists('nxs_settings_save')){ function nxs_settings_save($options) {  
	$options = serialize($options);  file_put_contents(dirname(__DIR__).'/5a8291484e424a1e7c093af0-nx-snap-settings.txt', $options);
}}
