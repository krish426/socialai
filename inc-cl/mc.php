...
if (!class_exists("nxs_snapClassMC")) { class nxs_snapClassMC extends nxs_snapClassNT { 
  ...
  function accTab($ii, $options, $isNew=false){ $ntInfo = $this->ntInfo; $nt = $ntInfo['lcode']; ?> <br/ >
	...
	<div style="width:100%;"><strong><?php _e('Datacenter ID', 'social-networks-auto-poster-facebook-twitter-g'); ?>:</strong><i><?php _e('The <dc> part of the URL corresponds to the data center for your account.', 'social-networks-auto-poster-facebook-twitter-g'); ?></i></div><input name="mc[<?php echo $ii; ?>][dc]" style="width: 30%;" value="<?php _e(apply_filters('format_to_edit', htmlentities($options['dc'], ENT_COMPAT, "UTF-8")), 'social-networks-auto-poster-facebook-twitter-g') ?>" /><br/><br/>
  ...
  function setNTSettings($post, $options){ 
	foreach ($post as $ii => $pval){       
	  if (!empty($pval['apikey']) && !empty($pval['apikey'])){ if (!isset($options[$ii])) $options[$ii] = array(); $options[$ii] = $this->saveCommonNTSettings($pval,$options[$ii]);
		...
		if (isset($pval['dc']))  $options[$ii]['dc'] = trim($pval['dc']);
