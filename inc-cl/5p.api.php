...
if (!class_exists("nxs_class_SNAP_5P")) { class nxs_class_SNAP_5P {
    ...
    function doPostToNT($options, $message){ global $nxs_urlLen; $badOut = array('pgID'=>'', 'isPosted'=>0, 'pDate'=>date('Y-m-d H:i:s'), 'Error'=>'');
      ...
      $url = 'https://upload.500px.com/v1/upload';  $fields = array( 'photo_id' => $upid, 'upload_key' => $uplk, 'consumer_key' => $options['appKey'], 'access_key' => $options['accessToken'] ); 
      $imgRes = nxs_curlUploadImg($imgURL, $url, $fields, 'file');
      // $imgRes = $imgURL;
