...
//================================Instagram===========================================
if (!class_exists('nxsAPI_IG')){class nxsAPI_IG{ var $ck = array(); var $agent=''; var $guid=''; var $phid='';  var $dId=''; var $debug = false; var $loc = ''; var $proxy = array();
    ...
    function check($u=''){ $ck = $this->ck; if (!empty($ck) && is_array($ck)) { $hdrsArr = $this->headers(''); if ($this->debug) echo "[IG] Checking....;<br/>\r\n";
        $advSet = nxs_mkRemOptsArr($hdrsArr, $ck, '', $this->proxy); $rep = nxs_remote_get('https://i.instagram.com/api/v1/accounts/me/', $advSet); 
        if (is_nxs_error($rep)) return false; $ck = $rep['cookies']; $contents = $rep['body']; //if ($this->debug) prr($contents);
        $ret = stripos($contents, 'href="#accountBasics"')!==false; $usr = CutFromTo($contents, '"email": "', '"'); if ($ret & $this->debug) echo "[PN] Logged as:".$usr."<br/>\r\n"; 
        $apVer = trim(CutFromTo($contents,'"app_version": "', '"'));  $this->apVer = $apVer; 
        if (empty($u) || $u==$usr) return $ret; else return false;
    ...
    function connect($u,$p){ $badOut = 'Error: '; // $this->debug = true;
      //## Check if alrady IN
      if (!$this->check($u)){ if ($this->debug) echo "[IG] NO Saved Data; Logging in...<br/>\r\n";
        ...
        //## ACTUAL LOGIN 
        $hdrsArr = $this->headers('', '', 'POST'); $hdrsArr['User-Agent']=$this->agent;  $advSet = nxs_mkRemOptsArr($hdrsArr, '', $flds, $this->proxy); 
        $rep = nxs_remote_post('https://i.instagram.com/api/v1/accounts/login/', $advSet); if (is_nxs_error($rep)) {  $badOut = print_r($rep, true)." - ERROR -02-"; return $badOut; }    if (empty($rep['body'])) {  $badOut = print_r($rep, true)." - ERROR -03-"; return $badOut; }    
        ...
        if ($obj['status']=='ok') { $ck = $rep['cookies']; foreach ($ck as $ci=>$cc) $ck[$ci]->value = urlencode($cc->value); $this->ck = $ck; return false; } else return "ERROR -POST- ".print_r($obj, true);  
      } else { if ($this->debug) echo "[PN] Saved Data is OK;<br/>\r\n"; return false; }
    }
